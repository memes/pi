...
func testBrutePrimeFindNext(start uint64, expected uint64, t *testing.T) {
	t.Parallel()
	if actual := BruteFindNextPrime(start); actual != expected {
...
func TestBrutePrimeFindNext(t *testing.T) {
	for i := uint64(0); i < primeVerifyLimit; i++ {
		...
		t.Run(fmt.Sprintf("start=%d", i), func(t *testing.T) {
			testBrutePrimeFindNext(i, expected, t)
		})
	}
}
...
// of 10.
func BenchmarkBrutePrimeFindNext(b *testing.B) {
